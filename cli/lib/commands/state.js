...
const { statePath, archiveDir } = require('../utils/paths');
...
function getState(opts) {
  const state = readJson(statePath, { allowMissing: true }) || {};
...
function updateState(phase, opts) {
  ...
  const current = readJson(statePath, { allowMissing: true }) || {};
...
function archiveState(force, opts) {
  ...
  const destDir = path.join(archiveDir, stamp);
  ensureDir(destDir);
  const destPath = path.join(destDir, 'state.json');
  try {
    fs.renameSync(statePath, destPath);
  } catch (err) {
    if (err && err.code === 'EXDEV') {
      fs.copyFileSync(statePath, destPath);
